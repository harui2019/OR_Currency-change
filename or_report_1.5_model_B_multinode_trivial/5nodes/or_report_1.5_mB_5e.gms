...
;
Parameters
    w_on(node) "" /a 1, b 0, c 0, d 0, e 0/,
    f_on(node) "" /a 0, b 0, c 0, d 0, e -1/,
    c(i,j) "" ,
...
*$ontext
*gdx2csv
$call gdxdump or_report.1.5_mB_5e.gdx symb=cost CSVSetText format=csv header = "cost(iter)">> or_report.1.5_mB_5e_out_cost.csv
$call gdxdump or_report.1.5_mB_5e.gdx symb=isfeasible CSVSetText format=csv header = "isfeasible(iter)">> or_report.1.5_mB_5e_out_isfeasible.csv
$call gdxdump or_report.1.5_mB_5e.gdx symb=remain CSVSetText format=csv header ="remain(iter)">> or_report.1.5_mB_5e_out_remain.csv
$call gdxdump or_report.1.5_mB_5e.gdx symb=remainrate CSVSetText format=csv header = "remainrate(iter)">> or_report.1.5_mB_5e_out_remainrate.csv
*$offtext